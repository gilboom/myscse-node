...
async function login(username, password) {
        ...
        await getLoginPage()
        await sendRequest()
        const $ = await getMainPage()
        const modules = getModules($)
        const uriList = getModulesUri(modules)
        // log(uriList)
        setContext({ uriList })
        return context
        ...
        async function getLoginPage() {
                const options = {
                        method: 'GET',
                        uri: 'http://class.sise.com.cn:7001/sise/',
                        encoding: null,
                ...
                const JSESSIONID = getJSESSIONID(cookie)
                setContext({ cookie, JSESSIONID })
                const jQuery = getJQuery(response.body)
                generateToken(jQuery, context.JSESSIONID)
                inputs = getInputs(jQuery)
                Object.assign(inputs, { username, password })

                function getInputs($) {
                        const _inputs = Array.from($('form input'))

                        const inputs = {}

                        _inputs.forEach(function (input) {
                                let { attribs: { name, value } } = input
                                inputs[name] = value
                        }, this)
                ...
                function generateToken(jQuery, JSESSIONID) {
                        var rand = jQuery("#random").val();
                        var src = "http://class.sise.com.cn:7001/sise/" + JSESSIONID + rand;
                        var md5 = jQuery.heheda(src).toLocaleUpperCase();
                        jQuery("#token").val(patch(md5, rand))
                        return jQuery
                        ...
                        function patch(f, e) {
                                ...
                                for (var b = d.length; b < g.length; b++) {
                                        a.push(g[b])
                                }
                                return a.join("")
                        }
        ...
        async function sendRequest() {
                const options = {
                        method: 'POST',
                        uri: 'http://class.sise.com.cn:7001/sise/login_check_login.jsp',
                        encoding: null,
                        form: inputs,
                        headers: {
        ...
        async function getMainPage() {
                const options = {
                        method: 'GET',
                        uri: 'http://class.sise.com.cn:7001/sise/module/student_states/student_select_class/main.jsp',
                        encoding: null,
                        headers: {
                                'Cookie': context.cookie,
                        },
                        transform(body) {
                                return getJQuery(body)
                        }
                }
                return await request(options)
        }
        ...
        function getModules($) {
                const modules = $('table[class="table1"]>tbody>tr>td>table td')
                return modules
        }

        function getModulesUri(modules) {
                let uriList = new Map()
                Array.from(modules).forEach(function (module) {
                        let moduleOnClickString = module.attribs.onclick
                        let moduleName;
                        if(module.children[4].firstChild === null) {
                                moduleName = module.children[6].firstChild.data
                        }else {
                                moduleName =  module.children[4].firstChild.data
                        }

                        if (moduleOnClickString === undefined) {
                                return uriList.set(moduleName,undefined)
                        } else {
                                let regex = /'.*'/
                                let hostname = "http://class.sise.com.cn:7001"
                                let uri = regex.exec(moduleOnClickString)[0].split("'")[1]
                                if (/\.\.\//.test(uri)) {
                                        uri = hostname + /\/SISEWeb.*/.exec(uri)[0]
                                } else {
                                        uri = hostname + uri
                                }
                                uriList.set(moduleName,uri)
                        }
                }, this)
                return uriList
        }
...
function initContext({ cookie, JSESSIONID }) {
        Object.assign(context, { cookie, JSESSIONID })
}
...
function getJSESSIONID(cookie) {
        return cookie.split('=')[1].split('!')[0]
}

function getJQuery(body) {
        const GBKBody = getGBKBody(body)
        const $ = cheerio.load(GBKBody)
        initJQuery($)
        return $

        function getGBKBody(body) {
